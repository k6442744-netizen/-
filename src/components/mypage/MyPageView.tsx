...
import { useNetwork } from "@/lib/past-life";
 ...
 * 마이페이지.
 *
 * 오버레이로 덮으면 뒤 화면이 비쳐 시선을 뺏겨서 독립된 화면으로 둔다.
 * 사주정보·하트 충전처럼 그 자리에서 끝나는 일만 팝업으로 띄운다.
 */
export function MyPageView() {
  ...
  /* 친구가 새로 참여했으면 여기서 바로 알 수 있게 한다 */
  const { newCount } = useNetwork(defaultProfile);
    ...
    <div className="space-y-4">
      ...
      <RetroWindow label="내 정보" tone="pink">
        <div className="flex items-center gap-3.5 px-4 pb-4 pt-4">
          <span className="flex size-14 shrink-0 items-center justify-center rounded-win border border-line bg-page-pink">
            <Icon name="user" size={26} className="text-accent" />
          </span>
      ...
      {/* 하트 잔액 */}
      <RetroWindow label="보유 하트" tone="pink">
      ...
      >
        <Icon name="star" size={20} className="shrink-0 text-brand-lav" />
        <span className="flex flex-1 items-center gap-1.5">
          <span className="dot-text text-[14px] text-ink">전생관계 판별기</span>
        ...
        {newCount > 0 ? (
          <span className="badge-pop shrink-0 rounded-full bg-brand-pink px-2 py-0.5 text-[10px] font-bold text-white">
            NEW {newCount}
          </span>
        ) : null}
      </Link>
            ...
            return (
              <li
                ...
                className={`${wide ? "col-span-2" : ""} ${
                  !wide && i % MENU_COLUMNS === 0 ? "border-r border-silver" : ""
      ...
      <div className="flex items-center justify-center gap-3 pb-2 pt-1">
        <button
          ...
          onClick={signOut}
          disabled={!session}
          className="text-[13px] text-ink-faint underline-offset-4 transition-colors hover:text-ink-soft hover:underline disabled:opacity-40 disabled:hover:no-underline"
        >
          로그아웃
        </button>
      ...
      />
      <HeartChargeModal open={chargeOpen} onClose={() => setChargeOpen(false)} />
    </div>
  );
}
